...
async function loginUser() {
  ...
  if (!selectedUserId) {
    setStatus("Оберіть cерверного користувача", "error");
    return;
  }
  ...
  try {
    ...
    if (isNetworkError(error)) {
      ...
      state.availableUsers = [];
      renderAvailableUsers();
      setStatus("Сервер недоступний", "error");
      return;
    }
...
function buildLocalItem(formData, user) {
  ...
  if (!ownerId) {
    throw new Error("Збережи користувача або увійди");
  }
